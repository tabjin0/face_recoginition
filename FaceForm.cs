...
namespace YunZhiFaceReco
{
    public partial class FaceForm : Form
    {
        ...
        public FaceForm()
        {
            ...
            /* ***********************************窗体启动自动打开摄像头视频框 start *************************************************  */
            //必须保证有可用摄像头
            if (filterInfoCollection.Count == 0)
            {
                MessageBox.Show("未检测到摄像头，请确保已安装摄像头或驱动!");
            }
            if (videoSource.IsRunning)//  摄像头未开启状态
            {
                btnStartVideo.Text = "启用摄像头";
                videoSource.SignalToStop(); //关闭摄像头
                chooseImgBtn.Enabled = true;// 选择图片按钮可用
                matchBtn.Enabled = true;// 比对头像按钮可用
                txtThreshold.Enabled = false;// 阈值设置为不可更改状态
                videoSource.Hide();// 摄像头视频框隐藏
            }
            else// 摄像头开启状态
            {
                if (isCompare)
                ...
                videoSource.Start();// 摄像头视频框启动
            }
            /* ***********************************窗体启动自动打开摄像头视频框 end *************************************************  */

            videoSource.Hide();// 窗体启动的时候隐藏摄像头视频框，并不是不运行，当检测到人脸的时候，显示摄像头视频框
